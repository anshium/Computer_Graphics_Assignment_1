...
void BVH_Node::createBVH(Scene scene){
	for(Surface surface : scene.surfaces){
		this->surfaces_inside.push_back(surface);
	}
...
void BVH_Node::reallyCreateBVH(){
	...
	for(Surface surface : this->surfaces_inside){
		this->node_bounding_box.update(surface.bounding_box.min);
		this->node_bounding_box.update(surface.bounding_box.max);
	}
	...
	double spitting_position = this->node_bounding_box.min[axis] + 0.5 * range[axis];
